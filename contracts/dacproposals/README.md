...
- approval_expiry - A timepoint when the approval window closes and the proposal can no longer be considered by the custodians. It is set when the proposal is created and is not extended when work starts, so a proposal that is in progress can be past this while its job, and the escrow holding the pay for it, are still live.
...
Proposals and their votes stay in the contract tables after they are no longer useful, so `clearexpprop` exists to remove an expired proposal and its votes and free up the RAM. Any account may call it: the action only succeeds when the proposal has already expired, so the outcome does not depend on who calls it.
